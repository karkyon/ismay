...
└── app/                        # Next.jsアプリ本体
    ...
    └── src/
        ...
        │   ├── patterns/           # Case Pattern Catalog(検出・照合・Suggestion生成・Feedback記録)
...
| 30日Purge：FKで到達できないデータ | **決定待ち(DEC-PURGE-02B)**。`consents`/`event_logs`/`outbox_events`/`jobs`/`audit_logs`(行為者参照はNULL化して行を保持)/`ai_runs`(capture無しの行)/Object Storage(MinIOの音声・画像本体)はPurgeで削除されない。dry-run・実行結果に保持表として明示する。選択肢と推奨は`docs/decisions/DEC-PURGE-02B.md` |
| 30日Purge：大量処理の運用台帳 | **未実装(PURGE-OPS-03)**。`--all`は逐次実行で、PurgeRun/PurgeItem台帳・lease・retry・dead-letter・中断再開は未実装 |
