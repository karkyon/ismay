...
import { deferFormationSession } from "@/lib/formation/sessionLifecycle";

/**
 * V5-M1-B6B: POST /formation-sessions/{id}/defer
 * 出典: Claude向け ISMAY 69b5a87以降 監査是正・M1-B6完遂・PEM-0連続実装指示
 *       (2026-08-31) Gate M1-B6B Session Lifecycle。
 * CLARIFYING/REVIEW_READY/PARTIALLY_CONFIRMED --DEFER_SESSION/DEFER_REMAINING--> DEFERRED。
 */
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const parsed = DeferRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  const result = await deferFormationSession({
    sessionId,
    workspaceId,
    ...
    switch (result.error) {
      ...
      case "INVALID_SESSION_STATE":
        return apiError(
          ...
          `このSessionは現在${result.sessionState}のためdeferできません(CLARIFYING/REVIEW_READY/PARTIALLY_CONFIRMEDのみ対象)`,
        );
